...
export const calculateAverageRating = (
  ...
  const totalStars = comments.reduce(
    (total: number, comment: ICommentProduct) => total + comment.rate,
    0
  );

  const averageStars = totalStars / comments.length;
  const roundedAverageStars = Number(averageStars.toFixed(decimalPlaces));

  const starIcons = Array.from({ length: 5 }, (_, index) => (
    <svg
      key={index}
      className={`text-${
        index < roundedAverageStars ? "yellow" : "gray"
      }-300 h-5 w-5 flex-shrink-0`}
      viewBox="0 0 20 20"
      fill="currentColor"
      aria-hidden="true"
    >
